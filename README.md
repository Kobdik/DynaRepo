...
Высокая скорость работы `SqlDataReader` обусловлена применением `FAST_FORWARD` курсора на стороне БД, однако, непосредственно использовать его неудобно, получается слишком много кода. Для доступа к данным обычно прибегают к помощи `EF` (Entity Framework), который внутренне использует `SqlDataReader`, но при этом инициализирует новые сущности, прибегая к рефлексии, что в конечном итоге приводит к существенному (десятикратному) снижению скорости при выборке данных из БД (Test1 - EF, Test2, Test3, Test4 - альтернативные подходы). Ситуацию не улучшить, если далее использовать стандартные подходы сериализации созданных объектов с использованием рефлексии. Сравнение скорости записи в выходной поток проведу в ближайшее время.
...
При первом проходе цикла итераций, сначала исполняется select-запрос к БД, возвращается `IDataReader` и далее каждая последующая итерация продвигается вместе с курсором БД. Данные кэшируются во внутреннем списке `List<T>`, так что повторные циклы итераций работают уже над кэшем, который при желании можно обновить.
...
private static void Test1()
{
 using (var context = new TestModel())
 {
  ...
  foreach (Invo invo in query)
  {
   ...
   // count, invo.Idn, invo.Dt_Invo, invo.Val, invo.Note);
  }
  ...
  Console.WriteLine("Done 0. Count={0}, Sum={1}. Time elapsed {2} ticks", 
   count, sum_gt, ts);
  Console.WriteLine("Total memory {0}", GC.GetTotalMemory(false));
  //На основании скорости повторного исполнения убеждаемся,
  //что данные кэшированы
  count = 0;
  sum_gt = 0;
...
static void Test2()
{
 ...
 //это в 5.76 раз быстрее, чем EF только считывает аналогичные данные из БД
