...
Высокая скорость работы `SqlDataReader` обусловлена применением `FAST_FORWARD` курсора на стороне БД, однако, непосредственно использовать его неудобно, получается слишком много кода. Для доступа к данным обычно прибегают к помощи `EF` (Entity Framework), который внутренне использует `SqlDataReader`, но при этом инициализирует новые сущности, прибегая к рефлексии, что в конечном итоге приводит к существенному (десятикратному) снижению скорости при выборке данных из БД (Test1 - EF, Test2, Test3 - альтернативные подходы). Ситуацию не улучшить, если далее использовать стандартные подходы сериализации созданных объектов с использованием рефлексии. Сравнение скорости записи в выходной поток проведу в ближайшее время.
